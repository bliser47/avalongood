...
class Assassination {
            ...
            if (socket && selectedPlayers) {
                ...
                if ( this.thisRoom.playersInGame )
                {
                    for (var i = 0; i < this.thisRoom.playersInGame.length; i++) {
                        if (this.thisRoom.playersInGame[i].role === 'Puck') {
                            puckExists = true;
                            break;
                        }
                    }
    ...
    buttonSettings(indexOfPlayer) {
        ...
        for (let i = 0; i < this.thisRoom.playersInGame.length; i++) {
            ...
            if (this.thisRoom.playersInGame[i].role === 'Isole') {
                isoldeExists = true;
            }
            if (this.thisRoom.playersInGame[i].role === 'Tristan') {
                tristanExists = true;
        ...
        if (indexOfPlayer === indexOfAssassin) {
            ...
            if ( guinevereExists ) {
                if ( merlinExists ) {
                    obj.green.setText = 'Shoot as Merlin';
                } else if ( tristanExists && isoldeExists ) {
                    obj.green.setText = 'Shoot as lovers';
                } else {
                    obj.green.hidden = true;
                }
                obj.red.hidden = false;
                obj.red.setText = 'Shoot as Guinevere';
            } else {
    ...
    numOfTargets(indexOfPlayer) {
        if (indexOfPlayer !== undefined && indexOfPlayer !== null) {
            ...
            if (this.thisRoom.playersInGame[indexOfPlayer].role.indexOf('Assassin') >= 0) {
                ...
                if ( (tristExists && isoExists) && (merlinExists || guinevereExists)) {
                    return [1,2];
                }
                if ( tristExists && isoExists ) {
                    return 2;
                }
                if ( merlinExists || guinevereExists ) {
                    return 1;
                }
                return killable;
            } else {
