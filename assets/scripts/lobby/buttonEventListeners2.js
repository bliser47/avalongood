...
function redButtonFunction() {
    ...
    if (document.querySelector('#red-button').classList.contains('disabled') === false) {
        ...
        } else if (gameData.phase === 'votingTeam' || gameData.phase === 'votingMission' || gameData.phase === 'assassination') {
            if ( gameData.phase === 'assassination' ) {
                socket.emit('gameMove', ['no', getHighlightedAvatars()]);
            } else {
